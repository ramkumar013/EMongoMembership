...
namespace ExtendedMongoMembership
{
    public class MongoMembershipProvider : ExtendedMembershipProvider
    {
        ...
        {
            throw new NotImplementedException("This method should not be implemented as Id is changed from Int to GUID.");
            //VerifyInitialized();
            //using (var session = new MongoSession(_connectionString))
            //{
        ...
        private void CreateUserRow(MongoSession session, string userName, IDictionary<string, object> values)
        {
            ...
            Guid userId = GetUserId(session, userName);
        ...
        private static int GetPasswordFailuresSinceLastSuccess(MongoSession session, Guid userId)
        {
        ...
        private bool CheckPassword(MongoSession session, Guid userId, string password)
        {
        ...
        private string GetHashedPassword(MongoSession session, Guid userId)
        {
            var user = session.Users.FirstOrDefault(x => x.UserId == userId);
        ...
        public  string GetUserNameFromId(Guid userId)
        {
            VerifyInitialized();
        ...
        public new Guid GetUserIdFromOAuth(string provider, string providerUserId)
        {
            VerifyInitialized();
        ...
        /// </returns>
        public bool HasLocalAccount(Guid userId)
        {
            VerifyInitialized();
